...
const getColor = (percentage, type, colors = {}) => {
  const defaultColor = {
    default: '#000',
    success: '#0070f3',
    secondary: '#666',
  ...
  const coustomColorKey = coustomColorKeys.find(key => percentage <= +key)
  if (!coustomColorKey || Number.isNaN(+coustomColorKey)) return defaultColor[type]
  return colors[+coustomColorKey]
}

export default React.memo(({ value = 0, max = 100, type = 'default', className, colors, children, ...props }) => {
